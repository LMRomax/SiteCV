<div class="intro-container">
    <div class="intro-container--lean"></div>
    <div class="intro-presentation">
        <div class="intro-presentation--content">
            <div class="intro-presentation--logo">
                <img src="assets/images/lm.webp" alt="Maxence Lemaître" width="100%" height="100%">
            </div>

            <div class="intro-presentation--name">
                Maxence Lemaître
            </div>
            <div class="intro-presentation--devweb">
                Développeur Web
            </div>
        </div>
    </div>

    <div class="arrow"></div>
</div>

<div class="menu--nav">
    <nav class="navigation-menu">
        <a href="javascript:void(0);">
            <span class="active" id="element-pres">Présentation</span>
        </a>

        <a href="javascript:void(0);" id="element-comp">
            <span>Compétences</span>
        </a>

        <a href="javascript:void(0);" id="element-portfolio">
            <span>Projets</span>
        </a>

        <a href="javascript:void(0);" id="element-xp">
            <span>Expériences</span>
        </a>

        <a href="javascript:void(0);" id="element-formations">
            <span>Formations</span>
        </a>

        <a href="javascript:void(0);" id="element-refs">
            <span>Références</span>
        </a>

        <a href="javascript:void(0);" id="element-contact">
            <span>Contact</span>
        </a>
    </nav>
</div>

<div class="responsive-mobile-nav">
    <nav class="navigation-menu navigation-menu--responsive">
        <a href="javascript:void(0);" id="mobile-pres" class="responsive--link-menu">
            <span class="active">Présentation</span>
        </a>

        <a href="javascript:void(0);" id="mobile-comp" class="responsive--link-menu">
            <span>Compétences</span>
        </a>

        <a href="javascript:void(0);" id="mobile-portfolio" class="responsive--link-menu">
            <span>Projets</span>
        </a>

        <a href="javascript:void(0);" id="mobile-xp" class="responsive--link-menu">
            <span>Experiences</span>
        </a>

        <a href="javascript:void(0);" id="mobile-formations" class="responsive--link-menu">
            <span>Formations</span>
        </a>

        <a href="javascript:void(0);" id="mobile-refs" class="responsive--link-menu">
            <span>Références</span>
        </a>

        <a href="javascript:void(0);" id="mobile-contact" class="responsive--link-menu">
            <span>Contact</span>
        </a>
    </nav>
</div>